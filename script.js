...
function displayProduct(products){   
       ...
       <svg xmlns="http://www.w3.org/2000/svg" class="N1bADF" width="16" height="16" viewBox="0 0 20 16"><path d="M8.695 16.682C4.06 12.382 1 9.536 1 6.065 1 3.219 3.178 1 5.95 1c1.566 0 3.069.746 4.05 1.915C10.981 1.745 12.484 1 14.05 1 16.822 1 19 3.22 19 6.065c0 3.471-3.06 6.316-7.695 10.617L10 17.897l-1.305-1.215z" fill="#c2c2c2" class="x1UMqG" stroke="#FFF" fill-rule="#c2c2c2" opacity=".9"></path></svg>
        ...
        <div class="phone-name" id="phoneName">${element.mobile_name} </div>`
    ...
    `  
       ...
       ` 
      const alignemnt=document.querySelectorAll(".align")
       console.log(alignemnt)
      const phoneNames=document.querySelectorAll(".phone-name")
      console.log(phoneNames)
      console.log(phoneNames)
      alignemnt.forEach((align,index)=>{
        align.addEventListener("mouseover",()=>{
           phoneNames[index].style.color="#2874f0"
        })
        align.addEventListener("mouseout",()=>{
           phoneNames[index].style.color="black"
        })  
      }) 
    })  
}
...
clearPrimary.addEventListener("click",()=>{
 ...
 popularity.addEventListener("click",()=>{ cccc
    displayProduct(allProducts)
 }) 
...
const thickLine=document.getElementById("thickLine")
const circleMin=document.getElementById("circleOne") 
const circleMax=document.getElementById("circleTwo");
const variationBoxes=document.querySelectorAll(".price-veriation")
const step=5;
const veriationPrice=Array.from(variationBoxes)
.map(box=>parseInt(box.dataset.price))
.sort((a,b)=>a-b) 
let currentMin=veriationPrice[0]; 
let currentMax=veriationPrice[veriationPrice.length-1];

function updateSlider(){ 
    const lineWidth=line.offsetWidth;
    const minPercent=((currentMin-veriationPrice[0])/(veriationPrice[veriationPrice.length-1]-veriationPrice[0]))*100
    const maxPercent=((currentMax-veriationPrice[0])/(veriationPrice[veriationPrice.length-1]-veriationPrice[0]))*100 
    const minPos=(minPercent/100)*lineWidth; 
    const maxPos=(maxPercent/100)*lineWidth; 
    circleMin.style.left=`${minPos}px`        
    circleMax.style.left=`${maxPos}px`  
    thickLine.style.left=`${minPos}px`  
    thickLine.style.width=`${maxPos-minPos}px`  

    const filterProduct=allProducts.filter(
       p=>p.d_price>=currentMin && p.d_price<=currentMax
    )  
    displayProduct(filterProduct) ;
   let filter = document.querySelector(".filter-style");
    if (!filter) {
        filter = document.createElement("div");
        filter.className = "filter-style";
        priceRange.appendChild(filter);
        filterClear.style.display="block"
    }    
    filter.innerHTML = `  <span>x</span><span class="title-space">${currentMin} - ${currentMax}</span>`; 
    filter.addEventListener("click",()=>{
        currentMax;
        currentMin; 
...
function getNearrestPrice(price){ 
    return veriationPrice.reduce((prev,curr)=>Math.abs(curr-prev)<Math.abs(prev-price)?curr:prev)
} 5
 function handleDrag(circle, isMin) { 
    const onMouseMove = (e) => {
        ...
        const rawPrice = veriationPrice[0] + (pos / rect.width) * (veriationPrice[veriationPrice.length - 1] - veriationPrice[0]);

        // Snap to the nearest variation price
        const newPrice = getNearestPrice(rawPrice);

        if (isMin) {
            if (newPrice >= currentMax) return;
            currentMin = newPrice;
            minPriceInput.value = currentMin;
        } else {
            if (newPrice <= currentMin) return;
            currentMax = newPrice;
            maxPriceInput.value = currentMax;
        }
...
circleMin.addEventListener("mousedown", () => handleDrag(circleMin, true));
circleMax.addEventListener("mousedown", () => handleDrag(circleMax, false));
 

minPriceInput.addEventListener("change",(e)=>{
    currentMin=parseInt(e.target.value);
    if(currentMin>=currentMax)currentMin=currentMax-step; 
    updateSlider();  
})
maxPriceInput.addEventListener("change",(e)=>{
    currentMax=parseInt(e.target.value);
    if(currentMax<=currentMin)currentMax=currentMin+step;
    updateSlider();
})
...
priceClear.addEventListener("click",()=>{
      ...
      updateSlider()
      displayProduct(allProducts)
      priceClear.style.display="block" 
})
const search=document.getElementById("Search")
const brands=document.getElementById("brand-name")
const label=document.querySelectorAll("#brand-name label")
search.addEventListener("keyup",()=>{
     const searchText=search.value.toLowerCase().trim()
     label.forEach(label=>{
        const labelElement=label.textContent.toLowerCase()
        if(labelElement.includes(searchText)){
